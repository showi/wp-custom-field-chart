...
$CFC_FIELDS = array(
	...
	new Field('period', true, 'month', '^(year|month|day)$',
	    'Period: year, month, day'),
	new Field('fields', true, Null, '^\w[\w\d_-]+$',
	    'Comma separated wordpress custom field that we want to plot'),
...
function cfc_collect_data($atts)
{
    ...
    if (isset($atts['to_date'])) {
        ...
        if (! $end_time) {
            return 'Tally Graph: couldn\'t read the to_date ' .
                $atts['to_date'];
        }
        // unset($atts['to_date']);
    } else
    ...
    if (isset($atts['method'])) {
        ...
        if (! in_array($method, array(
            ...
            CFC_GRAPH_DELTA_METHOD,
            CFC_GRAPH_TRACK_METHOD
        ))) {
